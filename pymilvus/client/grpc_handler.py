...
class GrpcHandler:
    ...
    def create_database(self, db_name, timeout=None):
        ...
        status = self._stub.CreateDatabase.future(request, timeout=timeout).result()
        if status.error_code != 0:
            raise MilvusException(status.error_code, status.reason)
    ...
    def drop_database(self, db_name, timeout=None):
        ...
        status = self._stub.DropDatabase.future(request, timeout=timeout).result()
        if status.error_code != 0:
            raise MilvusException(status.error_code, status.reason)
    ...
    def list_database(self, timeout=None):
        ...
        response = self._stub.ListDatabases.future(request, timeout=timeout).result()
        if response.status.error_code != 0:
            raise MilvusException(response.status.error_code, response.status.reason)
